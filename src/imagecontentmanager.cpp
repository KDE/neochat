...
{
    QFile file(":/emojis.data"_ls);
    file.open(QFile::ReadOnly);
    auto data = QString::fromUtf8(file.readAll());

    for (const auto &line : data.split(u'\n')) {
        if (line.startsWith("//"_ls) || line.isEmpty()) {
            continue;
        }
        if (line.startsWith("##"_ls)) {
            const auto &parts = line.mid(2).split(QLatin1Char(';'));
...
QString ImageContentManager::accountImagesAvatar() const
{
    ...
    if (avatarUrl.isEmpty()) {
        avatarUrl = Controller::instance().activeConnection()->user()->avatarUrl().toString();
    }
    if (avatarUrl.isEmpty()) {
        avatarUrl = event.images[0].url.toString();
    }
    return QStringLiteral("<img src=\"%1\" width=\"32\" height=\"32\"/>")
        .arg(Controller::instance().activeConnection()->makeMediaUrl(QUrl(avatarUrl)).toString());
}
