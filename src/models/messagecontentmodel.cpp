...
bool MessageContentModel::showReaction() const
{
    return m_showReaction;
}

void MessageContentModel::setShowReaction(bool show)
{
    if (show == m_showReaction) {
        return;
    }
    m_showReaction = show;
    if ((m_components.last().type == MessageComponentType::Reaction) != (m_showReaction || m_forceReaction)) {
        resetContent();
    }
    Q_EMIT showReactionChanged();
}

bool MessageContentModel::forceReaction() const
{
    return m_forceReaction;
}

void MessageContentModel::setForceReaction(bool force)
{
    if (force == m_forceReaction) {
        return;
    }
    m_forceReaction = force;
    if ((m_components.last().type == MessageComponentType::Reaction) != (m_showReaction || m_forceReaction)) {
        resetContent();
    }
    Q_EMIT forceReactionChanged();
}
    ...
    if (role == DisplayRole) {
        if (m_currentState == UnAvailable || m_room->connection()->isIgnored(senderId())) {
            ...
            return QString(u"<span style=\"color:%1\">"_s.arg(disabledTextColor)
                           + i18nc("@info", "This message was either not found, you do rootnot have permission to view it, or it was sent by an ignored user")
                           + u"</span>"_s);
        }
...
QList<MessageComponent> MessageContentModel::messageContentComponents(bool isEditing, bool isThreading)
{
    ...
    if ((m_reactionModel && m_reactionModel->rowCount() > 0) || m_forceReaction || m_showReaction) {
