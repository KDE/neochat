...
void NotificationsManager::startNotificationJob(QPointer<NeoChatConnection> connection)
{
    ...
    if (!m_connActiveJob.contains(connection->user()->id())) {
        ...
        connection->callApi<GetNotificationsJob>().onResult(this, [this, connection](const auto &job) {
                                            ...
                                            qint64 timestamp)
{
    ...
    notification->setHint(u"x-kde-symbolic-app-icon"_s, u"org.kde.neochat.tray"_s);
    ...
    if (canReply) {
        ...
        connect(replyAction.get(), &KNotificationReplyAction::replied, room, [room, replyEventId](const QString &text) {
