...
void ProfileFieldsHelper::load()
{
    ...
    m_connection->callApi<GetProfileFieldJob>(BackgroundRequest, m_userId, QStringLiteral("us.cloke.msc4175.tz"))
        .then(
            [this](const auto &job) {
    ...
    m_connection->callApi<GetProfileFieldJob>(BackgroundRequest, m_userId, QStringLiteral("com.redstrate.pronouns"))
        .then(
            [this](const auto &job) {
